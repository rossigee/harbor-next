...
func (n *webhookAPI) requireExecutionInPolicy(ctx context.Context, execID, policyID int64) error {
	...
	if exec.VendorID == policyID && (exec.VendorType == job.WebhookJobVendorType || exec.VendorType == job.SlackJobVendorType || exec.VendorType == job.AMQPJobVendorType || exec.VendorType == job.MatrixJobVendorType) {
		return nil
	}
