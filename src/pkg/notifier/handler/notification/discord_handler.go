...
func (d *DiscordHandler) process(ctx context.Context, event *model.HookEvent) error {
	if event.Payload == nil {
		return fmt.Errorf("invalid discord event: nil payload")
	}
	if event.Target == nil {
		return fmt.Errorf("invalid discord event: nil target")
	}
...
func (d *DiscordHandler) convert(payLoad *model.Payload) (string, error) {
	...
	if len(eventDataStr) > 1000 {
		eventDataStr = eventDataStr[:1000] + "\n... (truncated)"
	}
