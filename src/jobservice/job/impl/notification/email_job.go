...
func (ej *EmailJob) Validate(params job.Parameters) error {
	...
	for _, param := range requiredParams {
		...
		if value == nil {
			return errors.Errorf("malformed job parameter '%s', got nil", param)
		}
		str, ok := value.(string)
		if !ok || str == "" {
			return errors.Errorf("malformed job parameter '%s', expecting string but got %s", param, fmt.Sprintf("%T", value))
		}
	}
	return nil
}
...
func (ej *EmailJob) execute(params map[string]any) error {
	...
	if address == "" {
		return errors.New("email SMTP server not configured")
	}
	...
	ej.logger.Infof("sending email to %v via %s", to, address)
	...
	if err != nil {
		ej.logger.Errorf("failed to send email: %v", err)
		return errors.Wrap(err, "failed to send email")
	}

	ej.logger.Infof("email sent successfully")
	return nil
}
