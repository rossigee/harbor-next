...
func (tg *TelegramJob) Validate(params job.Parameters) error {
	...
	if text == nil {
		return errors.Errorf("malformed job parameter 'text', got nil")
	}
	str, ok := text.(string)
	if !ok || str == "" {
		return errors.Errorf("malformed job parameter 'text', expecting string but got %s", fmt.Sprintf("%T", text))
	}
	...
	if botToken == nil {
		return errors.Errorf("malformed job parameter 'bot_token', got nil")
	}
	str, ok = botToken.(string)
	if !ok || str == "" {
		return errors.Errorf("malformed job parameter 'bot_token', expecting string but got %s", fmt.Sprintf("%T", botToken))
	}
	...
	if chatID == nil {
		return errors.Errorf("malformed job parameter 'chat_id', got nil")
	}
	str, ok = chatID.(string)
	if !ok || str == "" {
		return errors.Errorf("malformed job parameter 'chat_id', expecting string but got %s", fmt.Sprintf("%T", chatID))
	}
	return nil
}
...
func (tg *TelegramJob) execute(params map[string]any) error {
	...
	if err != nil {
		return errors.New("error to send request to Telegram API")
	}
