	...
	if resp.StatusCode == http.StatusTooManyRequests {
		...
		if retryAfter != "" {
			if seconds, err := strconv.Atoi(retryAfter); err == nil && seconds > 0 {
				dj.logger.Infof("rate limited by Discord, waiting %d seconds", seconds)
				time.Sleep(time.Duration(seconds) * time.Second)
				return errors.New("rate limited by Discord, job should be retried")
			}
		}
		var respData map[string]any
		jsonBody, _ := io.ReadAll(resp.Body)
		_ = json.Unmarshal(jsonBody, &respData) // best-effort parse; empty respData falls through below
		if retryAfterVal, ok := respData["retry_after"]; ok {
			if seconds, ok := retryAfterVal.(float64); ok && seconds > 0 {
				dj.logger.Infof("rate limited by Discord, waiting %.0f seconds", seconds)
				time.Sleep(time.Duration(seconds) * time.Second)
				return errors.New("rate limited by Discord, job should be retried")
			}
		}
		dj.logger.Warning("rate limited by Discord but could not parse retry_after, using default 1 second")
